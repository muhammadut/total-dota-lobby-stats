...
def main():
    ...
    # W1-SAT-S1 is Team 1 vs Team 2 (slot 1); W1-SAT-S2 is Team 3 vs Team 4.
    s1 = series_by_id("W1-SAT-S1")
    t_a, t_b = s1["teams"]
    print(f"\n  using {s1['id']}: Team {t_a} vs Team {t_b} "
          f"({s1['start_utc']} .. {s1['end_utc']})")

    group("happy path")
    write_matches([make_match("g1", roster(t_a), roster(t_b), "radiant", at("W1-SAT-S1"))])
    info, errs = LR.resolve("g1", None)
    check("no errors", not errs, errs)
    check("picked W1-SAT-S1", info.get("series", {}).get("id") == "W1-SAT-S1",
    ...
    write_matches([make_match("g1", roster(t_a), roster(t_b), "dire", at("W1-SAT-S1"))])
    info, errs = LR.resolve("g1", None)
    check(f"winner is Team {t_b}", info.get("winner") == t_b, info.get("winner"))

    group("sides swapped still resolves to the same series")
    write_matches([make_match("g1", roster(t_b), roster(t_a), "radiant", at("W1-SAT-S1"))])
    info, errs = LR.resolve("g1", None)
    check("same series", info.get("series", {}).get("id") == "W1-SAT-S1", errs)
    ...
    write_matches([make_match("g1", mixed, roster(t_b), "radiant", at("W1-SAT-S1"))])
    ...
    check("refusal lists the options", any("W1-SAT-S1" in e for e in errs), errs)
    ...
    write_matches([make_match("g1", nicks, roster(t_b), "radiant", at("W1-SAT-S1"))])
    info, errs = LR.resolve("g1", None)
